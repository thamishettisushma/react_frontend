            ...
            <DialogContent className="w-[90vw] max-w-lg sm:max-w-xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={submitHandler} className="space-y-4">
                    <div className="grid gap-3">
                        {[
                            { id: "fullname", label: "Full Name", type: "text", value: input.fullname },
                            { id: "email", label: "Email", type: "email", value: input.email },
                            { id: "phoneNumber", label: "Phone Number", type: "text", value: input.phoneNumber },
                            { id: "bio", label: "Bio", type: "text", value: input.bio },
                            { id: "skills", label: "Skills", type: "text", value: input.skills, placeholder: "Comma-separated skills" }
                        ].map(({ id, label, type, value, placeholder }) => (
                            <div key={id} className="flex flex-col sm:flex-row sm:items-center gap-2">
                                <Label htmlFor={id} className="sm:w-1/3">{label}</Label>
                                <Input
                                    id={id}
                                    name={id}
                                    type={type}
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                            <Label htmlFor="file" className="sm:w-1/3">Resume</Label>
                            <div className="flex-1 space-y-2">
                                {/* Show existing resume link if available */}
                                {input.resumeURL && (
                                    <a href={input.resumeURL} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline block">
